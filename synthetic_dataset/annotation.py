...
#     print(ex)
# breakpoint()
feedback = rg.FeedbackDataset.from_argilla("midjourney-v0", workspace="admin")

data = {}
for record in tqdm.tqdm(feedback.records):
    try:
        data[record.external_id] = {
            "quality_1": str(record.responses[0].values["quality_1"].value),
            "quality_2": str(record.responses[0].values["quality_2"].value),
            "quality_3": str(record.responses[0].values["quality_3"].value),
            "quality_4": str(record.responses[0].values["quality_4"].value),
        }
    except Exception as ex:
        print(ex)

with open("midjourney-v0.json", "w") as f:
    json.dump(data, f)
